...
function DrawingGame() {
  ...
  // ✅ استخدام ref بدلاً من state لتجنب تأخير التحديث
  const isRestoringCanvasRef = useRef(false);
  const hasSetupSocketRef = useRef(false); // ✅ منع تسجيل listeners متعددة
  ...
  // ✅ إعداد Canvas محسّن مع حفظ الرسومات
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleRestoreCanvas = (data) => {
      ...
      // ✅ تفعيل وضع الاستعادة باستخدام ref
    ...
    const handleYourTurnToDraw = (data) => {
      ...
      setRoundEnded(false); // ✅ إعادة تفعيل التخمين للجولة الجديدة
      ...
      if (!data.isRejoining && !isRestoringCanvasRef.current) {
    ...
    const handleSomeoneDrawing = (data) => {
    ...
    const handleCorrectGuess = (data) => {
      const pointsText = data.pointsEarned ? ` (+${data.pointsEarned} نقطة)` : '';
      const timeText = data.timeElapsed ? ` بعد ${data.timeElapsed} ثانية` : '';
      
      setMessages(prev => [{
        type: 'correct',
        playerName: data.playerName,
        message: `✅ خمّن الكلمة بشكل صحيح!${timeText}${pointsText}`,
        id: Date.now()
      }, ...prev]);
    ...
    const handleRoundEnded = (data) => {
      ...
      setRoundEnded(true); // ✅ قفل حقل التخمين
      
      // رسالة نقاط الرسام
      let drawerMessage = '';
      if (data.drawerPoints !== undefined && data.drawerName) {
        const bonus = data.allGuessed ? ' (+ إجماع 🏆)' : '';
        drawerMessage = `🎨 ${data.drawerName} حصل على ${data.drawerPoints} نقطة${bonus}`;
      }
    ...
    socket.on('restore-canvas', handleRestoreCanvas);
    ...
    socket.on('your-turn-to-draw', handleYourTurnToDraw);
    socket.on('someone-drawing', handleSomeoneDrawing);
    ...
    socket.on('guesses-update', handleGuessesUpdate);
    socket.on('no-guesses-left', handleNoGuessesLeft);
    ...
    return () => {
      ...
      socket.off('restore-canvas', handleRestoreCanvas);
      ...
      socket.off('your-turn-to-draw', handleYourTurnToDraw);
      socket.off('someone-drawing', handleSomeoneDrawing);
      ...
      socket.off('guesses-update', handleGuessesUpdate);
      socket.off('no-guesses-left', handleNoGuessesLeft);
  ...
  const handleConfirmLeave = () => {
    ...
    console.log('🚪 المغادرة من لعبة الرسم');
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <input
                        ...
                        placeholder={
                          ...
                          remainingGuesses > 0 ? "اكتب تخمينك..." : "لا محاولات!"
                        }
                        ...
                        disabled={remainingGuesses <= 0 || roundEnded}
                        autoFocus
                      />
                      <button
                        type="submit"
                        disabled={!guess.trim() || remainingGuesses <= 0 || roundEnded}
                  ...
                  {messages.map((msg) => (
                    <div
                      ...
                      className={`p-2 sm:p-3 rounded-lg text-sm border ${
                        ...
                        msg.type === 'drawer' ? 'bg-pink-500/20 border-pink-500/30' :
                    ...
                    >
                      {msg.type === 'correct' ? (
                        <div>
                          ...
                          <p className="text-green-200 text-xs">{msg.message}</p>
                        </div>
                      ) : msg.type === 'info' ? (
                        ...
                        <p className="text-center font-bold text-pink-300">
                          {msg.message}
                        </p>
                      ) : msg.type === 'error' ? (
